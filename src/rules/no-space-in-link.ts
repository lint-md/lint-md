    ...
    return {
      link: (node: PositionedLinkNode) => {
        ...
        if (childTextNodeList.length === 1) {
          ...
          checkAndReportTextNode(context, childTextNodeList[0] as PositionedTextNode, 'between');
        }
          ...
          checkAndReportTextNode(context, startItem as PositionedTextNode, 'start-only');
          checkAndReportTextNode(context, endItem as PositionedTextNode, 'end-only');
        }
      }
    };
  }
